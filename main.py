...
ALLOWED_ROLE_ID = 1470456899911549095
...
class PollView(ui.View):
    def __init__(self, options: list, duration_seconds: int, question: str):
        ...
        for option in options:
            btn = ui.Button(label=f"{option} (0)", style=discord.ButtonStyle.primary, custom_id=f"poll_{option}")
    ...
    def make_callback(self, option_name: str):
        async def callback(interaction: discord.Interaction):
            ...
            self.user_choices[interaction.user.id] = (interaction.user.display_name, option_name)
            ...
            await interaction.followup.send(f"✅ Your vote for **{option_name}** has been recorded!", ephemeral=True)
    ...
    async def on_timeout(self):
        ...
        details = "\n".join([f"• **{name}**: {choice}" for name, choice in self.user_choices.values()]) if self.user_choices else "No votes submitted."
...
class GiveawayView(ui.View):
    ...
    @ui.button(label="Enter Giveaway 🎉", style=discord.ButtonStyle.success, custom_id="join_giveaway")
...
class SourcePaginationView(ui.View):
    ...
    async def send_current_page(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(content=header_text, attachments=[file], view=self)
        
        if os.path.exists(file_path):
            os.remove(file_path)

    @ui.button(emoji="⬅️", style=discord.ButtonStyle.secondary, custom_id="prev_page")
    ...
    @ui.button(label="1", style=discord.ButtonStyle.primary, disabled=True, custom_id="page_indicator")
    async def page_indicator(self, interaction: discord.Interaction, button: ui.Button):
        pass

    @ui.button(emoji="➡️", style=discord.ButtonStyle.secondary, custom_id="next_page")
...
async def prefix_search(ctx, *, query: str = ""):
    if not query:
        return await ctx.send("⚠️ Please specify a script name or query. Usage: `!search script_name`")

    if not saved_sources:
        return await ctx.send("❌ No sources stored yet!")

    q = query.lower().strip()
    matched = [s for s in saved_sources if q in s['name'].lower() or q in s.get('filename', '').lower()]
...
@bot.tree.command(name="reload", description="Reload saved sources from local storage")
async def reload_cmd(interaction: discord.Interaction):
    if not is_staff(interaction):
        return await interaction.response.send_message("You are not admin or staff", ephemeral=True)
    
    global saved_sources
    saved_sources = load_sources()
    await interaction.response.send_message(f"🔄 Reloaded sources! Total stored: **{len(saved_sources)}**", ephemeral=True)

@bot.tree.command(name="say", description="Make the bot send a custom message up to 5 times")
@app_commands.describe(message="The message to send", amount="Number of times (1 to 5)")
...
async def say(interaction: discord.Interaction, message: str, amount: int = 1):
    ...
    await interaction.followup.send(f"✅ Sent message **{amount}** time(s)!", ephemeral=True)

@bot.tree.command(name="take", description="Scrape and save ALL unique .txt/.lua sources in channel")
async def take(interaction: discord.Interaction):
    ...
    await interaction.followup.send(
        f"✅ Scraped and saved **{count}** new source(s)!\nTotal stored unique sources: **{len(saved_sources)}**",
        ephemeral=True
    )

@bot.tree.command(name="leak", description="Send a specified amount of saved sources randomly")
...
async def leak(interaction: discord.Interaction, amount: int):
    ...
    if not saved_sources:
        return await interaction.response.send_message("❌ No sources stored yet! Run `/take` first.", ephemeral=True)
    ...
    for src in to_send:
        ...
        if len(content) > 1900:
            ...
            with open(file_p, "w", encoding="utf-8") as f:
                f.write(content)
            await interaction.channel.send(file=discord.File(file_p, filename=f"{name}.txt"))
            if os.path.exists(file_p):
                os.remove(file_p)
        else:
            await interaction.channel.send(ticks + nl + content + nl + ticks)

        sent_count += 1
        await asyncio.sleep(1)

    is_leaking = False
    await interaction.followup.send(f"✅ Finished leaking **{sent_count}** source(s)!", ephemeral=True)

@bot.tree.command(name="stop", description="Stop the current running leak process")
async def stop(interaction: discord.Interaction):
    ...
    if is_leaking:
        is_leaking = False
        await interaction.response.send_message("🛑 Stopping leak process...", ephemeral=True)
    else:
        await interaction.response.send_message("⚠️ No active leak process is running.", ephemeral=True)

@bot.tree.command(name="poll", description="Create a poll with duration in minutes")
...
async def poll(
    ...
    options = [opt for opt in raw_options if opt is not None and opt.strip() != ""]

    embed = discord.Embed(
        title=f"📊 {question}",
        description=f"Select an option below! This poll will end in **{duration_minutes}** minute(s).",
        color=discord.Color.blue()
    )

    view = PollView(options, duration_minutes * 60, question)
    await interaction.response.send_message(embed=embed, view=view)
    view.message = await interaction.original_response()

@bot.tree.command(name="giveaway", description="Start a giveaway in a target channel")
@app_commands.describe(
    prize="Giveaway Prize / Name",
    duration_minutes="Duration in minutes",
    hosted_by="Host mention / name",
...
async def giveaway(
    ...
    view = GiveawayView(duration_minutes * 60)
    ...
    if view.participants:
        ...
        await channel.send(f"🎉 Congratulations {winner_mention}! You won **{prize}**!")
    else:
        end_embed = discord.Embed(
            title=f"🎉 GIVEAWAY ENDED: {prize}",
...
@bot.tree.command(name="send_dm", description="Send a Direct Message to a specific user")
...
async def send_dm(interaction: discord.Interaction, user: discord.User, message: str):
    ...
    try:
        ...
        await interaction.response.send_message(f"✅ Successfully sent DM to **{user.name}**!", ephemeral=True)
    except Exception as e:
        await interaction.response.send_message(f"❌ Failed to send DM to {user.name}: {e}", ephemeral=True)

if __name__ == "__main__":
    # التوكن المدمج مباشرة في الكود
    token = "MTQ4ODU3MjU4MDI1MTA0NTg4OA.Gbnc3A.tMdIpg60xGf1TpCg-rL3nkxDJfBMPTpjcjZhRk"
    if token:
        bot.run(token)
    else:
